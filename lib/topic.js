...
exports.page = function (request, response) {
    var id = request.params.topicId; /* url.parse(_url, true).query; */
    // db.query('SELECT * FROM topic', function (error, topics) {
    //     if (error) {
    //         throw error;
    //     }
        db.query(`SELECT t.title as title
                       , t.description
                       , a.name as name
                    FROM topic as t
                    LEFT JOIN author as a
                      ON t.author_id = a.id
                   WHERE t.id=?`, [id], function (error2, topic) {
            if (error2) {
                throw(error2);
            } else {

                response.render('topic', {title: `${topic[0].title} - ${topic[0].name}`, topics:topic});
